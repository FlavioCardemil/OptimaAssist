...
import { crearMedico } from "@/app/actions/admin";
...
export default function ModalCrearMedico() {
  ...
  const [campos, setCampos] = useState({ nombre: "", especialidad: "", email: "", password: "" });
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await crearMedico(campos);
      if (result.error) { setError(result.error); }
      else { setAbierto(false); setCampos({ nombre: "", especialidad: "", email: "", password: "" }); router.refresh(); }
    });
  }
            ...
            <div className="om-modal-header">
              <div>
                ...
                <p style={{ fontFamily: "var(--font-outfit-var, sans-serif)", fontSize: "13px", color: "#9B9188", marginTop: "3px" }}>
                  Se creará cuenta en Auth y perfil profesional
                </p>
              </div>
              ...
              <div className="om-modal-body">
                <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                  <div>
                    ...
                    <input value={campos.nombre} onChange={(e) => set("nombre", e.target.value)} required className="om-input" placeholder="Dr. Juan Pérez" />
                  </div>
                  <div>
                    <label className="om-label">Especialidad <span style={{ color: "#DC2626", marginLeft: "2px" }}>*</span></label>
                    <input value={campos.especialidad} onChange={(e) => set("especialidad", e.target.value)} required className="om-input" placeholder="Medicina General" />
                  </div>
                  <div>
                    <label className="om-label">Email <span style={{ color: "#DC2626", marginLeft: "2px" }}>*</span></label>
                    <input value={campos.email} onChange={(e) => set("email", e.target.value)} required type="email" className="om-input" placeholder="medico@clinica.cl" />
                  </div>
                  <div>
